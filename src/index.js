...
async function dashboard(request, env, session, url) {
  ...
  for (const vehicle of vehicles) {
    ...
    rows.push(`<tr class="member" data-member data-name="${esc(`${vehicle.name} ${vehicle.code} ${vehicle.account}`.toLowerCase())}" data-category="${esc(vehicle.category)}"><td data-label="账号"><div class="member-title"><span class="avatar">${esc(vehicle.name.slice(0, 1).toUpperCase() || "?")}</span><span><strong>${esc(vehicle.name)}</strong><span class="sub">${vehicle.account ? esc(vehicle.account) : "未填写账号"}${vehicle.code ? ` · ${esc(vehicle.code)}` : ""}</span></span></div></td><td data-label="标签"><span class="badge">${esc(vehicle.category)}</span></td><td data-label="验证码"><button type="button" class="totp otp-copy" data-code-id="${vehicle.id}" data-plain-code="" title="点击复制验证码">••• •••</button><div class="sub" data-remaining-id="${vehicle.id}">正在获取</div></td><td data-label="分享链接"><input type="hidden" value="${esc(shareUrl)}" data-copy><div class="row-actions link-actions"><button type="button" class="copy-link" data-copy-button><span class="icon">↗</span>分享链接</button><form class="action-form" method="post" action="/admin/vehicles/${vehicle.id}/rotate" onsubmit="return confirm('旧链接将立即失效，确定重置？')"><input type="hidden" name="csrf" value="${esc(session.csrf)}"><button class="danger reset-link"><span class="icon">↻</span>重置链接</button></form></div></td><td data-label="操作"><div class="row-actions"><form class="action-form" method="post" action="/admin/vehicles/${vehicle.id}/toggle"><input type="hidden" name="csrf" value="${esc(session.csrf)}"><button class="ghost icon-btn" title="${vehicle.enabled ? "停用分享" : "启用分享"}"><span class="icon">${vehicle.enabled ? "⏸" : "▶"}</span></button></form><form class="action-form" method="post" action="/admin/vehicles/${vehicle.id}/delete" onsubmit="return confirm('确定删除此账号及访问记录？')"><input type="hidden" name="csrf" value="${esc(session.csrf)}"><button class="danger icon-btn" title="删除"><span class="icon">×</span></button></form></div></td></tr>`);
  }
  const options = categories.map((category) => `<option value="${esc(category.name)}">${esc(category.name)}</option>`).join("");
  const body = `${messageBlock(url)}<section class="kpi-grid"><div class="card kpi"><span>账号总数</span><b>${vehicles.length}</b></div><div class="card kpi"><span>分享链接</span><b>${active?.count || 0}</b></div><div class="card kpi"><span>标签数量</span><b>${categories.length}</b></div><div class="card kpi"><span>今日访问</span><b>${views?.count || 0}</b></div></section><div class="layout-grid"><section><div class="card toolbar"><input id="search" placeholder="搜索账号、标签、内容..."><select id="category-filter"><option value="">全部标签</option>${options}</select><span class="badge"><span id="visible-count">${vehicles.length}</span> 项</span></div><div class="card table-card"><div class="table-head"><h2>账号列表</h2><span class="sub">点击验证码数字可复制 · 每 30 秒自动更新</span></div><div class="table-wrap">${rows.length ? `<table><thead><tr><th>账号</th><th>标签</th><th>验证码</th><th>分享链接</th><th>操作</th></tr></thead><tbody id="members">${rows.join("")}</tbody></table>` : `<div class="empty"><strong>还没有账号</strong><br>用右侧表单添加第一个 TOTP 账号。</div>`}</div></div></section><aside class="card form-card" id="add-member"><h2>添加账号</h2><form method="post" action="/admin/vehicles"><input type="hidden" name="csrf" value="${esc(session.csrf)}"><label>名称</label><input name="name" required maxlength="80" placeholder="Google"><label>账号</label><input name="account" maxlength="160" placeholder="520.szxus@gmail.com"><label>编号</label><input name="code" required maxlength="20" placeholder="001"><label>标签</label><select name="category">${options}</select><label>Base32 密钥</label><input name="secret" required autocomplete="off" placeholder="JBSWY3DPEHPK3PXP"><label>备注</label><textarea name="notes" maxlength="2000" placeholder="保存后生成分享链接。"></textarea><button class="primary" style="width:100%;margin-top:14px"><span class="icon">+</span>保存并生成链接</button></form><hr id="categories" style="border:0;border-top:1px solid var(--border);margin:20px 0"><h3>标签分类</h3><form method="post" action="/admin/categories"><input type="hidden" name="csrf" value="${esc(session.csrf)}"><label>标签名称</label><input name="name" required maxlength="80"><label>颜色</label><select name="color"><option value="green">绿色</option><option value="blue">蓝色</option><option value="orange">橙色</option><option value="purple">紫色</option><option value="gray">灰色</option></select><button style="width:100%;margin-top:12px">添加标签</button></form></aside></div><section class="card form-card" id="settings" style="margin-top:16px"><h2>系统设置</h2><p class="muted">点击验证码复制、重置链接二次确认、右上角日夜模式切换已开启。</p></section>`;
